...
const resendFrom =
  Deno.env.get("RESEND_FROM") || "SportsLadder <no-reply@auth.sportsladder.nl>";
...
serve(async (req) => {
  ...
  const mode =
    payload?.mode === "second_friday" || payload?.mode === "start"
      ? payload.mode
      : "start";
  ...
  const ladderIds = Array.from(
    new Set(safeMatches.map((m) => m.ladder_id).filter(Boolean))
  ) as string[];
  ...
  const matchesByLadder = safeMatches.reduce<Record<string, typeof safeMatches>>(
    (acc, match) => {
  ...
  const playerIds = Array.from(
    new Set(
      safeMatches.flatMap((m) => [m.challenger_id, m.challenged_id]).filter(Boolean)
    )
  ) as string[];
  ...
  const primaryByPlayer: Record<string, string> = {};
  (membershipRows || []).forEach((row) => {
    partnerByPrimary[row.player_id] = row.partner_id ?? null;
    primaryByPlayer[row.player_id] = row.player_id;
    if (row.partner_id) {
      primaryByPlayer[row.partner_id] = row.player_id;
    }
  });
  ...
  for (const ladderId of Object.keys(matchesByLadder)) {
    ...
    const shouldSend =
      mode === "start"
        ? roundStart === today
        : isSecondFriday(roundStart, today);
  ...
  const emailsToSend = Array.from(recipientMatches.entries()).filter(([id]) => {
    return !!playerById[id]?.email;
  });
  ...
  for (const [recipientId, items] of emailsToSend) {
    ...
    const listItems = items
      .map((item) => {
        const statusLabel =
          item.status === "scheduled" ? "scheduled (overdue)" : item.status;
    ...
    const text = `Hi ${name},\n\nYou have matches to schedule or complete:\n${items
      .map((item) => {
        const statusLabel =
          item.status === "scheduled" ? "scheduled (overdue)" : item.status;
        const when = item.scheduledDate
          ? ` on ${formatDate(new Date(item.scheduledDate))}`
          : "";
        return `- vs ${item.opponent} (${item.ladderName}${item.roundLabel ? ` • ${item.roundLabel}` : ""} • ${statusLabel}${when})`;
      })
